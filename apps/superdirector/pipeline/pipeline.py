...
def _compute_run_status(framed: list[TopicPayload], errors: list[str]) -> str:
    fatal_errors = [e for e in errors if not _is_non_fatal_collector_issue(e)]
    if fatal_errors:
        return "partial"
    if len(framed) < TOP_N:
        return "partial"
    if any((t.get("errors") or []) or t.get("status") == "failed" for t in framed):
        return "partial"
    return "ok"
...
async def run_pipeline(app_state) -> dict[str, Any]:
    ...
    try:
        ...
        analyzed, analyze_errors, usage_a = await _run_step(
            "Analyzer",
            analyze_topics(app_state.ext_client, app_state.int_client, collected),
            timeout_seconds=PIPELINE_STEP_TIMEOUT_SECONDS,
        )
        ...
        framed, frame_errors, usage_f = await _run_step(
            "Framer",
            generate_frames(app_state.ext_client, app_state.int_client, top_scored),
            timeout_seconds=PIPELINE_STEP_TIMEOUT_SECONDS,
        )
        errors.extend(frame_errors)
        framed = _apply_frame_quality_gate(framed)
        ...
        async with AsyncSessionLocal() as session:
            ...
            prompt_tokens, output_tokens = _extract_tokens(usage_a)
            if prompt_tokens is not None or output_tokens is not None:
                ...
                cost_a = estimate_cost_usd(
                    usage_a.get("provider"),
                    usage_a.get("model"),
                    prompt_tokens,
                    output_tokens,
                )
                await crud.log_cost(
                    session,
                    provider=usage_a.get("provider") or "unknown",
                    model=usage_a.get("model") or GEMINI_FLASH_MODEL,
                    endpoint=usage_a.get("endpoint") or "unknown",
            ...
            prompt_tokens, output_tokens = _extract_tokens(usage_f)
            if prompt_tokens is not None or output_tokens is not None:
                total_prompt_tokens += prompt_tokens or 0
                total_output_tokens += output_tokens or 0
                cost_f = estimate_cost_usd(
                    usage_f.get("provider"),
                    usage_f.get("model"),
                    prompt_tokens,
                    output_tokens,
                )
                await crud.log_cost(
                    session,
                    provider=usage_f.get("provider") or "unknown",
                    model=usage_f.get("model") or GEMINI_FLASH_MODEL,
                    endpoint=usage_f.get("endpoint") or "unknown",
                    input_tokens=prompt_tokens,
                    output_tokens=output_tokens,
                    cost_usd=cost_f,
                )
            ...
            bitable_result = await _run_step(
                "Bitable sync",
                sync_topics_to_bitable(app_state.ext_client, framed),
                timeout_seconds=PIPELINE_STEP_TIMEOUT_SECONDS,
            )
            ...
            run_status = "quality_gate_failed" if not gate_passed else _compute_run_status(framed, errors)
            if not gate_passed:
